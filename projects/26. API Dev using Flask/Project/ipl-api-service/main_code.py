...
ipl_ball_localPath = os.path.join(BASE_DIR, "data", "ipl-balls.csv")

matches = pd.read_csv(ipl_matches_localPath)
balls = pd.read_csv(ipl_ball_localPath)
...
bowler_data["bowler_run"] = bowler_data[["extra_type", "total_run"]].apply(
    bowlerRun, axis=1
)
...
bowler_data["isBowlerWicket"] = bowler_data[["kind", "isWicketDelivery"]].apply(
    bowlerWicket, axis=1
)
...
def bowlerRecord(bowler, df):
    ...
    best_wicket = (
        gb.sort_values(["isBowlerWicket", "bowler_run"], ascending=[False, True])[
